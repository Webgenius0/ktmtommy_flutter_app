...
//   String selectedInjury = 'Injury 1'; // Injury Name
//   String selectedInjuryLevel = 'Mid'; // Injury Level
//   int selectedIndex = -1; // Recovery stage index
//
//   @override
//   void dispose() {
...
//                               log('⚙️ Injury Level: $selectedInjuryLevel');
//                               log('📅 Injury Date: ${dateController.text}');
...
//
//
//                               log('+++++++++Injury Name: ${appData.read(kKeyInjuryName)}');
//                               log('+++++++++Injury Level: ${appData.read(kKeyInjuryLevel)}');
//                               log('+++++++++Injury Date: ${appData.read(kKeyInjuryDate)}');
//                               log('+++++++++Current Recovery Stage: ${appData.read(kKeyRecoveryStage)}');
//
...
// ধরে নিচ্ছি এগুলো আপনার প্রজেক্টে সঠিকভাবে সংজ্ঞায়িত আছে
import 'package:ktmtommy_apps/assets_helper/app_colors.dart';
import 'package:ktmtommy_apps/assets_helper/app_fonts.dart';
import 'package:ktmtommy_apps/assets_helper/app_icons.dart';
import 'package:ktmtommy_apps/common_widgets/custom_arrow_back.dart';
...
// TbiCustom উইজেটের একটি উদাহরণ সংজ্ঞা (যদি আপনার প্রজেক্টে না থাকে)
...
class _RecoveryStepOneScreenState extends State<RecoveryStepOneScreen> {
  ...
  // তারিখের ব্যবধান গণনার ফাংশন
  String calculateAgeDifference(String dateString) {
    try {
      final List<String> dateParts = dateString.split('/');
      final DateTime injuryDate = DateTime(
      ...
      int years = currentDate.year - injuryDate.year;
      int months = currentDate.month - injuryDate.month;
      int days = currentDate.day - injuryDate.day;

      if (days < 0) {
        months -= 1;
        days += DateTime(injuryDate.year, injuryDate.month + 1, 0).day;
      }
      if (months < 0) {
        years -= 1;
        months += 12;
                        ...
                        CustomButtonWidget(
                          ...
                          onTap: () {
                            if (_formKey.currentState?.validate() ?? false) {
                              ...
                              log('🩹 Injury Name: $selectedInjury');
                              log('⚙️ Injury Level: $selectedInjuryLevel');
                              log('📅 Injury Date: ${calculateAgeDifference(dateController.text)}');
                              log('🧩 Current Recovery Stage: ${titles[selectedIndex]}');
                              log('==============================');
